...
const FeatureBadge = ({ label, value }: { label: string; value: string }) => {
  return (
    <div
      className="hand-drawn-badge border-2 p-4 transition-all duration-300 hover:shadow-md"
      style={{
      ...
      <div className="relative z-10">
        ...
        <section className="mx-auto max-w-7xl px-4 py-6 md:px-6">
          <div className="grid gap-8 lg:grid-cols-[380px_1fr]">
            ...
            {/* 左側控制台 */}
            <aside
              className="hand-drawn-aside-border relative overflow-hidden border-2 p-6 backdrop-blur-xl transition-all duration-500 shadow-lg hover:shadow-xl h-fit"
              style={{
                borderColor: "var(--app-border)",
            ...
            >
              ...
              <div className="mt-6 flex flex-wrap gap-2.5">
                ...
                <button
                  ...
                  className="rounded-full border px-4 py-3 text-sm font-semibold transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-40 border-[var(--app-border)] text-[var(--app-text)] hover:border-[var(--app-accent)] hover:text-[var(--app-accent)]"
                  style={{ backgroundColor: "color-mix(in srgb, var(--app-surface) 92%, white)" }}
                >
                  停止
                </button>
              ...
              {/* 音訊回放裝置 */}
              {audioUrl && (
                <div className="mt-5 rounded-[20px] border px-4 py-4 border-[var(--app-border)] animate-in slide-in-from-bottom-3 duration-300"
                  style={{ backgroundColor: "color-mix(in srgb, var(--app-surface) 92%, white)" }}
                >
            ...
            <div className="space-y-6">
              ...
              <motion.div
                ...
                className="overflow-hidden rounded-[28px] border-2 p-4 transition-all duration-300 md:p-6 border-[var(--app-border)]"
                style={{
                  boxShadow: "var(--app-shadow)",
                  backgroundColor: "color-mix(in srgb, var(--app-card) 92%, white)",
                }}
              ...
              <div className="grid grid-cols-2 gap-4 xl:grid-cols-3">
                <FeatureBadge label="花瓣數 (頻率群)" value={features ? `${features.petalCount} 瓣` : "--"} />
                <FeatureBadge label="高頻明亮度" value={features ? `${Math.round(features.treble * 100)}%` : "--"} />
                <FeatureBadge label="基調圓潤度" value={features ? `${Math.round(features.roundness * 100)}%` : "--"} />
                <FeatureBadge label="總體聲波能量" value={features ? `${Math.round(features.energy * 100)}%` : "--"} />
                <FeatureBadge label="細微粒子數" value={features ? `${features.particleCount} 點` : "--"} />
                <FeatureBadge label="生境多樣度" value={features ? `${Math.round(features.variance * 100)}%` : "--"} />
              </div>

              {/* 聲景描述與下載橫幅 */}
              <div
                className="hand-drawn-card-border relative overflow-hidden border-2 p-6"
                style={{
                  borderColor: "var(--app-border)",
                  boxShadow: "var(--app-shadow)",
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 relative z-10">
                  <div className="flex-1 min-w-0">
                    <h3 className="text-lg font-bold tracking-wide text-[var(--app-text)] flex items-center gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-[var(--app-accent)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
